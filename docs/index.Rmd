...
file_to_create %>% # work with the file_to_create variable
  tibble %>% # convert to tibble format 
  rename('filename' = '.') %>% # rename the default column name of '.' into 'filename'
  mutate(folder = paste0('folder_', str_extract(string=filename, pattern = '^[[:letter:]]{1}'))) # create a new column by combining the string 'folder_' and the first letter of the file names together
